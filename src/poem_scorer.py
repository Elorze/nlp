...
诗词评分技术流程图：
输入诗词 -> 分词处理 -> 多维度评分 -> 综合得分
  ↓
意境评分    主题评分    韵律评分    结构评分
(意象词)    (BERT)     (拼音)     (结构)

技术说明：
1. 使用 uer/gpt2-chinese-poem 预训练模型
   - 使用 BertTokenizer 进行分词
   - 使用 BertModel 进行语义理解
2. 评分维度：
   - 意境评分（40分）：使用意象词库评估
     * 意象词库：包含自然、植物、动物、情感、动作等类别的常用意象词
     * 评估方式：检查意象词使用是否得当，与上下文是否协调
   - 主题评分（25分）：使用 BERT 计算句子相似度
     * 计算过程：将每句诗转换为向量，计算相邻句子向量的余弦相似度
     * 评分标准：相似度越高，说明主题越连贯
     * 向量生成：BERT通过预训练学习，将每个字映射到包含语义、语法等特征的高维向量
     * 相似度计算：使用 cosine_similarity 计算向量间的余弦相似度
   - 韵律评分（20分）：使用 pypinyin 检查押韵
     * 检查方法：获取每行最后一个字的拼音韵母，比较是否相同
     * 评分标准：完全押韵20分，部分押韵5分，不押韵0分
   - 结构评分（15分）：检查字数和结构
3. 使用 jieba 进行中文分词
"""
